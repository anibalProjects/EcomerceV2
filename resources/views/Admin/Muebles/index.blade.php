                        ...
                        <tr>
                            ...
                            <td style="border: 1px solid #ddd; padding: 8px;">
                                @if($mueble->imagen_principal)
                                    <img src="{{ asset('storage/muebles/' . $mueble->imagen_principal) }}" alt="{{ $mueble->nombre }}" style="width: 50px; height: 50px; object-fit: cover;">
                                @else
                                    <span>Sin imagen</span>
                                @endif
                            </td>
                            <td style="border: 1px solid #ddd; padding: 8px;">{{ $mueble->nombre }}</td>
                            <td style="border: 1px solid #ddd; padding: 8px;">{{ $mueble->categoria ? $mueble->categoria->nombre : 'Sin categoría' }}</td>
                            <td style="border: 1px solid #ddd; padding: 8px;">{{ $mueble->precio }}€</td>
                            <td class="actions-cell" style="border: 1px solid #ddd; padding: 8px;">
                                <a href="{{ route('admin.muebles.edit', [$mueble->id, 'sesionId' => $sesionId]) }}" class="btn btn-edit">Editar</a>
                                <a href="{{ route('admin.muebles.galeria', [$mueble->id, 'sesionId' => $sesionId]) }}" class="btn btn-secondary">Galería</a>
                                <form action="{{ route('admin.muebles.destroy', [$mueble->id, 'sesionId' => $sesionId]) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
